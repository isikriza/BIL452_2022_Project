...
Overall, this assignment has the following components, and we provide scaffolding code in `sender_reciver`.
...
1. If it receives a packet with `seq_num` not equal to `N`, it will send back an `ACK` with `seq_num=N`. Note that this is slightly different from the Go-Back-N (GBN) mechanism. GBN totally discards out-of-order packets, while here `receiver` buffers out-of-order packets. The mechanism here is more efficient than GBN.
...
* **Please closely follow updates on Piazza**. All further clarifications will be posted on Piazza via Instructor Notes. We recommend you **follow** these notes to receive updates in time.
* You **MUST NOT** use TCP sockets.
* We are **NOT** providing any test cases. You can write your own test scripts if you want.
